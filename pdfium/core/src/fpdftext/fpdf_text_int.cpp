...
FX_BOOL CPDF_TextPage::ParseTextPage()
{
    ...
    m_IsParsered = FALSE;
    m_TextBuf.Clear();
    m_charList.RemoveAll();//init with len 512 
    m_pPreTextObj = NULL;
    ProcessObject();
    m_IsParsered = TRUE;
        ...
        for(int i = 0; i < nCount; i++) 
		{
            ...
            if(bNormal) 
			{
				...
				else 
				{
					...
					auto value = m_CharIndex.GetAt(indexSize - 1) + 1;
                    m_CharIndex.SetAt(indexSize - 1, value);
                }
            } 
			else 
			{
    ...
    while (pos) {
        ...
        if(pPageObj) {
            if(pPageObj->m_Type == PDFPAGE_TEXT) {
                ...
                // ProcessFormObject((CPDF_FormObject*)pPageObj, formMatrix); //zhiyong gank here
            }
        }
        pPageObj = NULL;
    }
...
void CPDF_TextPage::ProcessTextObject(PDFTEXT_Obj Obj)
{
	...
	for (int i = 0; i < nItems; i++) 
	{
		...
		Acc_GetCharBBox2(charinfo.m_CharCode, rect);
		//charinfo.m_pTextObj->GetFont()->GetCharBBox(charinfo.m_CharCode, rect);

		//char buff[100];
		//snprintf(buff, 100, "%d,  %d, %d, %d, %d\n", charinfo.m_CharCode,
